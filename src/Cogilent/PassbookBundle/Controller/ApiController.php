...
namespace Cogilent\PassbookBundle\Controller;
...
use Cogilent\PassbookBundle\Entity\mgeneral;
use Cogilent\PassbookBundle\Entity\mappearance;
use Cogilent\PassbookBundle\Entity\mdatasetting;
use Cogilent\PassbookBundle\Entity\mheader;
use Cogilent\PassbookBundle\Entity\mprimary;
use Cogilent\PassbookBundle\Entity\msecondary;
use Cogilent\PassbookBundle\Entity\mauxiliary;
use Cogilent\PassbookBundle\Entity\mbackfields;
use Cogilent\PassbookBundle\Entity\mrelevance;
use Cogilent\PassbookBundle\Entity\mdistribution;
use Cogilent\PassbookBundle\Base\PassHelper;
use Cogilent\PassbookBundle\Base\Files;
...
class ApiController extends FOSRestController
{
        ...
        $queryArray = $this->getDoctrine()->getManager()
            ->createQuery(
                'SELECT f.userId as UserId,e.couponBarcodeValueOption as Barcode, e.couponId as passSize,h.id as Id ,f.categoryName as Category,
                 ...
                 FROM CogilentPassbookBundle:mdatasetting e
                 JOIN CogilentPassbookBundle:mgeneral f where e.couponId = f.couponId
                 JOIN CogilentPassbookBundle:mdistribution g where f.couponId = g.couponId
                 JOIN CogilentPassbookBundle:mappearance h where g.couponId = h.couponId
    ...
    public function getpassnameformatAction($userid){
        ...
        $query = $em->createQuery('SELECT COUNT(g.id) FROM CogilentPassbookBundle:mgeneral g WHERE g.userId = '.$userid.' ');
        ...
        $queryArray = $this->getDoctrine()->getManager()
            ->createQuery(
                'SELECT f.userId as UserId,e.couponBarcodeValueOption as Barcode, e.couponId as passSize,h.id as Id ,f.categoryName as Category,
                 ...
                 FROM CogilentPassbookBundle:mdatasetting e
                 JOIN CogilentPassbookBundle:mgeneral f where e.couponId = f.couponId AND f.userId = '.$userId.'
                 JOIN CogilentPassbookBundle:mdistribution g where f.couponId = g.couponId
                 JOIN CogilentPassbookBundle:mappearance h where g.couponId = h.couponId
                 ORDER BY f.passGenerationDate DESC'
              )->getResult();
    ...
    public function getpassbyidAction($id){
        ...
        $em = $this->getDoctrine()->getManager();
        $general = $em->getRepository('CogilentPassbookBundle:mgeneral')->findOneBy(array('couponId' => $id));
        $appearance = $em->getRepository('CogilentPassbookBundle:mappearance')->findOneBy(array('couponId' => $id));
        $datasetting = $em->getRepository('CogilentPassbookBundle:mdatasetting')->findOneBy(array('couponId' => $id));
        $header = $em->getRepository('CogilentPassbookBundle:mheader')->findOneBy(array('couponId' => $id));
        $primary = $em->getRepository('CogilentPassbookBundle:mprimary')->findOneBy(array('couponId' => $id));
        $secondary = $em->getRepository('CogilentPassbookBundle:msecondary')->findOneBy(array('couponId' => $id));
        $auxiliary = $em->getRepository('CogilentPassbookBundle:mauxiliary')->findOneBy(array('couponId' => $id));
        $backfields = $em->getRepository('CogilentPassbookBundle:mbackfields')->findOneBy(array('couponId' => $id));
        $relevance = $em->getRepository('CogilentPassbookBundle:mrelevance')->findOneBy(array('couponId' => $id));
        $distribution = $em->getRepository('CogilentPassbookBundle:mdistribution')->findOneBy(array('couponId' => $id));
...
//Create Pass
    ...
    public function createpassuploadAction(Request $request){
        ...
        if(!empty($userEmail)){
            ...
            $em = $this->getDoctrine()->getManager();
            $task1 = $em->getRepository('CogilentPassbookBundle:mgeneral')->findOneBy(array('couponId' => $generalid ));
    ...
    public function modifypassAction(Request $request , $couponid){
        ...
        $general = $em->getRepository('CogilentPassbookBundle:mgeneral')->findOneBy(array('couponId' => $couponid ));
        $appearance = $em->getRepository('CogilentPassbookBundle:mappearance')->findOneBy(array('couponId' => $couponid));
        $datasetting = $em->getRepository('CogilentPassbookBundle:mdatasetting')->findOneBy(array('couponId' => $couponid ));
        $header = $em->getRepository('CogilentPassbookBundle:mheader')->findOneBy(array('couponId' => $couponid ));
        $primary = $em->getRepository('CogilentPassbookBundle:mprimary')->findOneBy(array('couponId' => $couponid ));
        $secondary = $em->getRepository('CogilentPassbookBundle:msecondary')->findOneBy(array('couponId' => $couponid ));
        $auxiliary = $em->getRepository('CogilentPassbookBundle:mauxiliary')->findOneBy(array('couponId' => $couponid ));
        $backfields = $em->getRepository('CogilentPassbookBundle:mbackfields')->findOneBy(array('couponId' => $couponid ));
        $relevance = $em->getRepository('CogilentPassbookBundle:mrelevance')->findOneBy(array('couponId' => $couponid ));
        $distribution = $em->getRepository('CogilentPassbookBundle:mdistribution')->findOneBy(array('couponId' => $couponid ));
    ...
    public function modifypassuploadAction(Request $request){
        ...
        if(!empty($userEmail)){
            ...
            $em = $this->getDoctrine()->getManager();
            $task1 = $em->getRepository('CogilentPassbookBundle:mgeneral')->findOneBy(array('couponId' => $coupon_id ));
    ...
    public function publishpassAction(Request $request , $status){
        ...
        $data = $request->request->all();
        $em = $this->getDoctrine()->getManager();
        $distribution = $em->getRepository('CogilentPassbookBundle:mdistribution')->findOneBy(array('couponId' => $data['coupon_id'] ));
        if ($status == 1){
    ...
    public function removepassAction(Request $request){
        ...
        foreach($delCouponIds as $coupon_id){
            $uploadDirId = 0;
            $em = $this->getDoctrine()->getManager();
            $general = $em->getRepository('CogilentPassbookBundle:mappearance')->findOneBy(array('couponId' => $coupon_id));
            $uploadDirId = $general->getId();

            $query = $em->createQuery('DELETE from CogilentPassbookBundle:mgeneral at where at.couponId Like :title');
            $query->setParameter('title', $coupon_id);
            $query->execute();
            $query1 = $em->createQuery('DELETE from CogilentPassbookBundle:mappearance bt where bt.couponId Like :title');
            $query1->setParameter('title', $coupon_id);
            $query1->execute();
            $query2 = $em->createQuery('DELETE from CogilentPassbookBundle:mdatasetting ct where ct.couponId Like :title');
            $query2->setParameter('title', $coupon_id);
            $query2->execute();
            $query3 = $em->createQuery('DELETE from CogilentPassbookBundle:mheader dt where dt.couponId Like :title');
            $query3->setParameter('title', $coupon_id);
            $query3->execute();
            $query4 = $em->createQuery('DELETE from CogilentPassbookBundle:mprimary et where et.couponId Like :title');
            $query4->setParameter('title', $coupon_id);
            $query4->execute();
            $query5 = $em->createQuery('DELETE from CogilentPassbookBundle:msecondary ft where ft.couponId Like :title');
            $query5->setParameter('title', $coupon_id);
            $query5->execute();
            $query6 = $em->createQuery('DELETE from CogilentPassbookBundle:mauxiliary gt where gt.couponId Like :title');
            $query6->setParameter('title', $coupon_id);
            $query6->execute();
            $query7 = $em->createQuery('DELETE from CogilentPassbookBundle:mbackfields ht where ht.couponId Like :title');
            $query7->setParameter('title', $coupon_id);
            $query7->execute();
            $query8 = $em->createQuery('DELETE from CogilentPassbookBundle:mrelevance iit where iit.couponId Like :title');
            $query8->setParameter('title', $coupon_id);
            $query8->execute();
            $query9 = $em->createQuery('DELETE from CogilentPassbookBundle:mdistribution jt where jt.couponId Like :title');
    ...
    public function generatePassAction($id) {
        ...
        $em     = $this->getDoctrine()->getManager();
        $task1  = $em->getRepository('CogilentPassbookBundle:mgeneral')->findOneBy(array('couponId' => $id));
        $task2  = $em->getRepository('CogilentPassbookBundle:mappearance')->findOneBy(array('couponId' => $id));
        $task3  = $em->getRepository('CogilentPassbookBundle:mdatasetting')->findOneBy(array('couponId' => $id));
        $task4  = $em->getRepository('CogilentPassbookBundle:mheader')->findOneBy(array('couponId' => $id));
        $task5  = $em->getRepository('CogilentPassbookBundle:mprimary')->findOneBy(array('couponId' => $id));
        $task6  = $em->getRepository('CogilentPassbookBundle:msecondary')->findOneBy(array('couponId' => $id));
        $task7  = $em->getRepository('CogilentPassbookBundle:mauxiliary')->findOneBy(array('couponId' => $id));
        $task8  = $em->getRepository('CogilentPassbookBundle:mbackfields')->findOneBy(array('couponId' => $id));
        $task9  = $em->getRepository('CogilentPassbookBundle:mrelevance')->findOneBy(array('couponId' => $id));
        $task10 = $em->getRepository('CogilentPassbookBundle:mdistribution')->findOneBy(array('couponId' => $id));
        ...
        $backfielddefault->setValue("This pass has been created and issued by a mCoupons user from ".$task1->getOrganizationName()." with the following e-mail : ".$task1->getUserEmail()." (the 'Pass Issuer') "."

        This pass has been created using a third party service, mCoupons, that is owned and operated by:

        mCoupons Aps
        Dianavej 30
        2610 Rødovre
        Denmark
        http://www.mcoupons.dk
        support@mcoupons.dk

        The Pass Issuer has attested that any trademarks and/or copyrighted content contained within this pass are being used with the consent of the owner.

        Questions, complaints, or claims with respect to this pass should always and only be addressed to the Pass Issuer.

        NEITHER APPLE NOR MCOUPONS SHALL BE LIABLE FOR ANY DAMAGES OR LOSSES ARISING FROM ANY USE, DISTRIBUTION, MISUSE, RELIANCE ON, INABILITY TO USE, INTERRUPTION, SUSPENSION, OR TERMINATION OF PASSBOOK, THIS PASS, OR ANY SERVICES PROVIDED IN CONNECTION THEREWITH, INCLUDING BUT NOT LIMITED TO ANY LOSS OR FAILURE TO DISPLAY PASS IN PASSBOOK OR ANY CLAIM ARISING FROM ANY USE OF THE FOREGOING BY YOU, THE PASS USER. THE SAME WILL BE VALID FOR ANY OTHER APP USED TO DISPLAY THIS PASS.");
    ...
    public function replicateAction($id)
    {
        ...
        $task11 = $em->getRepository('CogilentPassbookBundle:mgeneral')->findOneBy(array('couponId' => $id));
        $task22 = $em->getRepository('CogilentPassbookBundle:mappearance')->findOneBy(array('couponId' => $id));
        $task33 = $em->getRepository('CogilentPassbookBundle:mdatasetting')->findOneBy(array('couponId' => $id));
        $task44 = $em->getRepository('CogilentPassbookBundle:mheader')->findOneBy(array('couponId' => $id));
        $task55 = $em->getRepository('CogilentPassbookBundle:mprimary')->findOneBy(array('couponId' => $id));
        $task66 = $em->getRepository('CogilentPassbookBundle:msecondary')->findOneBy(array('couponId' => $id));
        $task77 = $em->getRepository('CogilentPassbookBundle:mauxiliary')->findOneBy(array('couponId' => $id));
        $task88 = $em->getRepository('CogilentPassbookBundle:mbackfields')->findOneBy(array('couponId' => $id));
        $task99 = $em->getRepository('CogilentPassbookBundle:mrelevance')->findOneBy(array('couponId' => $id));
        $task110 = $em->getRepository('CogilentPassbookBundle:mdistribution')->findOneBy(array('couponId' => $id));
    ...
    public function getPassJsonByIdAction($id)
    {
        ...
        $storeArray = Array();
        $storeArray[] = "http://mcoupons.cogilent.net/bundles/cogilentpassbook/pkpass/".$id.".pkpass";
        $dir    = $this->get('kernel')->getRootDir() . '/logs/pkpass';//'app/logs/pkpass';
        $files2 = scandir($dir);
        $files1 = array_diff($files2, array('.', '..'));
        $root = $this->get('kernel')->getRootDir();
        $src    = $root.'/logs/pkpass';
        $dst    = '/var/www/html/deploy/apppassbook/src/Cogilent/PassbookBundle/Resources/public/pkpass';
        //$dst    = '/var/www/html/apppassbook/src/Cogilent/PassbookBundle/Resources/public/pkpass';
        $dir = opendir($src);
        @mkdir($dst, 0777, true);
            ...
            $query = $this->getDoctrine()->getEntityManager()
                ->createQuery(
                    "SELECT e.id as ID,e.logoText as Title,e.iconName as Icon,e.couponId as PassID,
                     ...
                     FROM CogilentPassbookBundle:mappearance e
                     JOIN CogilentPassbookBundle:mprimary d where e.couponId = d.couponId
                     JOIN CogilentPassbookBundle:mgeneral f where d.couponId = f.couponId
                     JOIN CogilentPassbookBundle:mrelevance g where f.couponId = g.couponId
                     JOIN CogilentPassbookBundle:mdistribution h where g.couponId = h.couponId and h.distributionStatus = 1
                    ...
                    $query = $this->getDoctrine()->getEntityManager()
                        ->createQuery(
                            'SELECT e.name as categoryname
                             FROM CogilentPassbookBundle:category e
                             where e.id = ?1'
                        );
